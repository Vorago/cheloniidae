...
import cheloniidae.*;
import cheloniidae.attributes.Named;
import cheloniidae.commands.*;
import cheloniidae.predicates.HasAttribute;
import cheloniidae.predicates.Negation;
import cheloniidae.replicators.BandSplitReplicator;
import cheloniidae.replicators.InductiveReplicator;
import cheloniidae.transformations.Compose;
import cheloniidae.transformations.Identity;
import cheloniidae.transformations.Scale;
...
public abstract class CoreCommands {
    ...
    public static Move move(final double distance) {
        return new Move(distance);
    }

    public static Jump jump(final double distance) {
        return new Jump(distance);
    }

    public static Turn turn(final double angle) {
        return new Turn(angle);
    }

    public static Bank bank(final double angle) {
        return new Bank(angle);
    }

    public static Pitch pitch(final double angle) {
        return new Pitch(angle);
    }

    public static LineSize size(final double size) {
        return new LineSize(size);
    }

    public static LineColor color(final Color color) {
        return new LineColor(color);
    }

    public static Visible visible(final boolean visible) {
    ...
    public static Position position(final Vector position) {
        return new Position(position);
    }

    public static Direction direction(final Vector direction) {
    ...
    public static NullCommand pass() {
        return new NullCommand();
    }

    public static Debug debug(final String text) {
    ...
    public static Sequence sequence(final TurtleCommand... commands) {
        return new Sequence(commands);
    }

    public static Triangle triangle(final SupportsPosition p1, final SupportsPosition p2) {
        return new Triangle(p1, p2);
    }

    public static Triangle triangle(final TurtleCommand c1, final TurtleCommand c2) {
        return new Triangle(c1, c2);
    }
    ...
    public static Scale scale(final double factor) {
        return new Scale(factor);
    }

    public static Scale scale(final double factor, final boolean scaleLineSize) {
        return new Scale(factor, scaleLineSize);
    }
    ...
    public static double random(final double scale) {
        return random() * scale;
    }
    ...
    public static AddAttribute addAttribute(final Attribute a) {
        return new AddAttribute(a);
    }
    ...
    public static Pause pause(final long time) {
    ...
    public static NonDistributiveProxy atomic(final TurtleCommand c) {
        return new NonDistributiveProxy(c);
    }

    public static TriangleEmitter.Start triangleStart() {
        return new TriangleEmitter.Start();
    }

    public static TriangleEmitter.Emit triangleEmit() {
        return new TriangleEmitter.Emit();
    }
}
