...
public abstract class EuclideanTurtle<T extends EuclideanTurtle>
    ...
    public T position(final Vector _position) {
        position.assign(_position);
        return (T) this;
    }
    ...
    public T size(final double _size) {
        size = _size;
        return (T) this;
    }
    ...
    public T lineSize(final double _size) {
        return size(_size);
    }
    ...
    public T color(final Color _color) {
        color = _color;
        return (T) this;
    }
    ...
    public T lineColor(final Color _color) {
        return color(_color);
    }
    ...
    public T visible(final boolean _visible) {
        visible = _visible;
        return (T) this;
    }
    ...
    public T jump(final double distance) {
        position.addScaled(this.direction(), distance);
        return (T) this;
    }

    public T move(final double distance) {
        final Vector oldPosition = position.clone();
        return line(oldPosition,
                position.addScaled(this.direction(), distance));
    }

    public T line(final Vector p1, final Vector p2) {
        lines.add(new CartesianLine(p1, p2, size, color));
        return (T) this;
    }
    ...
    public SortedSet<RenderAction> actions(final Viewport v) {
        ...
        for (final RenderAction r : lines)
    ...
    public State serialize() {
        return new State(attributes, position, size, color);
    }

    public T deserialize(final TurtleState t) {
        if (t instanceof TurtleCommand)
    ...
    public String toString() {
        return "Euclidean turtle: location = " + position().toString() +
    ...
    public static class View extends ViewportCaching implements RenderAction {
        ...
        public View(final EuclideanTurtle _turtle) {
            turtle = _turtle;
        }
