...
public final class Vector implements Serializable, Cloneable, Comparable<Vector> {
    ...
    public Vector(final Vector existing) {
        this(existing.x, existing.y, existing.z);
    }

    public Vector(final double all) {
        this(all, all, all);
    }
    ...
    public Vector assign(final Vector v) {
        return this.assign(v.x, v.y, v.z);
    }

    public Vector assign(final double nx, final double ny, final double nz) {
        x = nx;
        y = ny;
        z = nz;
        return this;
    ...
    public Vector addScaled(final Vector other, final double factor) {
        x += other.x * factor;
        y += other.y * factor;
        z += other.z * factor;
        return this;
    ...
    public Vector componentwiseMinimum(final Vector other) {
        return this.assign(other.x < this.x ? other.x : this.x,
                other.y < this.y ? other.y : this.y,
                other.z < this.z ? other.z : this.z);
    }

    public Vector componentwiseMaximum(final Vector other) {
        return this.assign(other.x > this.x ? other.x : this.x,
                other.y > this.y ? other.y : this.y,
                other.z > this.z ? other.z : this.z);
    }
    ...
    public Vector proj(final Vector base) {
        return new Vector(base).multiply(this.dot(base) / base.lengthSquared());
    }

    public Vector orth(final Vector base) {
        return this.proj(base).multiply(-1.0).add(this);
    }
    ...
    public double distanceFrom(final Vector other) {
