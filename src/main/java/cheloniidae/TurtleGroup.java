...
public class TurtleGroup<T extends Turtle>
    ...
    public TurtleGroup(final T... _turtles) {
        for (final T t : _turtles) turtles.add(t);
    }

    public TurtleGroup(final Collection<T> _turtles) {
    ...
    public List<T> turtles() {
        return turtles;
    }

    public TurtleGroup<T> turtles(final List<T> _turtles) {
        turtles = _turtles;
        return this;
    }

    public TurtleGroup<T> window(final TurtleWindow _window) {
    ...
    public SortedSet<RenderAction> actions(final Viewport v) {
        ...
        for (final T t : turtles)
            for (final RenderAction r : t.actions(v))
